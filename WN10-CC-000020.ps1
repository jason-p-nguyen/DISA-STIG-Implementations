...
.DESCRIPTION
    Implements STIG control WN10-CC-000020 by setting the registry value:
    HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Tcpip6\Parameters\DisableIpSourceRouting = 2

    This configuration helps mitigate spoofing and man-in-the-middle attacks by disallowing source-routed packets.
...
try {
    ...
    if ($currentValue -eq $desiredValue) {
        Write-Host "[SUCCESS] IPv6 source routing is disabled (value = 2)." -ForegroundColor Green
    } else {
        Write-Host "[FAILURE] Value was not set correctly." -ForegroundColor Red
    }

} catch {
    Write-Error "An error occurred while applying WN10-CC-000020: $_"
}

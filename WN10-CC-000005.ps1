 <#
.SYNOPSIS
    Disables camera access on the Windows lock screen.

.DESCRIPTION
    Implements STIG control WN10-CC-000005 by setting the registry value:
    HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\Personalization\NoLockScreenCamera = 1

    This ensures that the lock screen camera is disabled, enhancing physical access control.
...
try {
    ...
    if ($currentValue -eq $desiredValue) {
        Write-Host "[SUCCESS] Lock screen camera is disabled." -ForegroundColor Green
    } else {
        Write-Host "[FAILURE] Value was not set correctly." -ForegroundColor Red
    }

} catch {
    Write-Error "An error occurred while applying WN10-CC-000005: $_"
}
